...
| `~/.claude/skills/manage-nodeterm-canvas/`, `~/.claude/skills/get-linked-context/`, `~/.pi/agent/skills/manage-nodeterm-canvas/`, `~/.pi/agent/skills/get-linked-context/` | nodeterm-owned skill dirs | Deleted |
...
| `~/.pi/agent/extensions/nodeterm-status.js` (or `$PI_CODING_AGENT_DIR/extensions/…` when set) | nodeterm-owned pi extension — pi has no hook FILE, only this JS extension module (first-line marker checked before deletion, same as the opencode plugin) | Deleted |
