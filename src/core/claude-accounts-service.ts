  ...
  return {
    ...
    [IPC.claudeAccountsRemove]: async (id: string, ctx?: AccountCtx) => {
      ...
      if (row?.host) {
        ...
        // LINKED account: the config dir is the USER's own (`~/.claude-2` with their real login in
        // it). Removing only forgets the row — an `rm -rf` would delete a second Claude identity the
        // app never created. Skip the teardown and fall through to deleteRow (the shell still owns
        // membership, so the row is dropped either way).
        if (!linkedClaudeConfigDirFor(id)) {
          // `accountConfigDir`, NOT `claudeConfigDirFor`: the resolver now answers with a LINKED dir
          // for a linked account, and the one recursive delete in this file must be incapable of
          // naming anything but `{userData}/claude-accounts/<id>` — id-validated, as it always was.
    ...
    /**
     * Link a PRE-EXISTING Claude config dir as an account. MEMBERSHIP is the shell's (see the file
     * header): the row is registered HERE through `settings.mutate`, NOT left for the renderer's
     * snapshot save — `configDir` is not a `RENDERER_OWNED_FIELD`, so a snapshot-appended linked row
     * would be reconciled straight back out (settings-store.ts `reconcileOwnedAccountList`). The
     * renderer mirrors the returned record optimistically; the row already exists on disk.
     *
     ...
     */
    [IPC.claudeAccountsLink]: async (rawDir: string) => {
      ...
      // Register the row on the store's chain (shell owns membership, like add/remove). Named by its
      // login when the dir is signed in, else the dir's own basename; the renderer's snapshot then
      // supplies the final label/email (both RENDERER_OWNED), but the row is on disk without it.
      const id = randomUUID()
      const account: ClaudeAccount = {
        id,
        label: email ?? path.basename(configDir),
        ...(email ? { email } : {}),
        configDir,
        createdAt: Date.now()
      }
      await registerRow(account)
      return { id, configDir, email }
    },

    /**
     * Turn `~/.claude/skills` sharing on/off for one LOCAL account and reconcile now (issue #643).
     * The flag is SHELL-owned: `shareSystemSkills` is not a `RENDERER_OWNED_FIELD`, so it is
     * persisted HERE through `settings.mutate` (a renderer snapshot save would be reconciled back
     * out); `applySkillShare` is the filesystem effect.
     *
     ...
     */
    [IPC.claudeAccountsSetSkillSharing]: async (id: string, enabled: boolean) => {
      ...
      const result = await applySkillShare(claudeConfigDirFor(id), enabled)
      // Persist the flag on the store's chain when the effect actually ran. A snapshot save cannot
      // change `shareSystemSkills` (reconcileOwnedAccountList keeps only label/color/email), so the
      // shell is the only writer — matching add/remove membership ownership.
      if (!result.refused) {
        await settings.mutate((s) => ({
          ...s,
          claudeAccounts: s.claudeAccounts.map((a) =>
            a.id === id ? { ...a, shareSystemSkills: enabled } : a
          )
        }))
      }
      return result
