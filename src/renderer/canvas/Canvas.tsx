...
// Delete a session from the board — same confirm + teardown as the canvas Delete key.
    ...
    return api.onAgentControl(async ({ requestId, sourceNodeId, verb, args }) => {
      ...
      if (verb === 'open-project') {
        ...
        setConfirm({
          ...
          requestedBy: opTitle,
          // NO `waiveVerb` (open-project is outside CONFIRM_WAIVABLE_VERBS): it registers a new
          // directory and records a grant, and is already deduped per (caller, project), so it
          // cannot produce the dialog storm the waiver exists to end. Still a deadline, so an
          // abandoned dialog cannot hold `confirmBusy` for the app run.
      ...
      const liveSurface: ControlSurface = {
        ...
        addRope: (source, target) =>
          setControlEdges((es) => [...es, ropeEdge(`ctrl-${source}-${target}`, source, target)]),
        pruneRopes: (gone) =>
      ...
      // `settings` (get/set a small set of settings from an agent) does its OWN live/store lookup
      // and returns before the surface routing below: a get is read-only, a set asks with a
      // per-request confirm (never waivable) and applies through the SAME setters the Settings UI
      // uses (lib/settingsVerb). The view never switches on its behalf — an off-screen source is
      // resolved from the store, like every other verb.
      if (verb === 'settings') {
      ...
      // ── end of the `settings` verb (own live/store lookup, self-expiring confirm, never waived) ──
      ...
      // command-delivery refuses a truncated line rather than submitting half of it. The spilled
      // file is a real upload (`files.saveUpload`), so an off-screen launch that armColdOpenHere
      // holds in `pendingLaunch` keeps a `promptFile` path that still resolves when the project is
      // next shown — the surface's `ctlSsh`/`ctlProject` are the TARGET project's, so the SSH
      // exclusion is evaluated against the machine the pane will actually run on.
      ...
      const resolveAfter = (): string[] | null | undefined => {
        ...
        // `--after a,a` is ONE wait: a duplicate id would write two dep ropes for one pair that
        // collide on a single `ctrl-<dep>-<node>` id (see ropeDeps).
      ...
      // it has launched (displayEdges derives that from pendingLaunch via ropeVisual). Written at
      // spawn so the waiting arrow appears immediately, not only after the next project load (the
      // `missingDepRopes` healer covers that late case). The opener's own rope is skipped — it
      // already exists, and ropeVisual renders it waiting when the node waits on the opener itself.
      //
      // Through `surface.addRope`, NEVER a bare `setControlEdges`: upstream wrote these ropes
      // straight to the active canvas' control edges (it had no off-screen surface), which would
      // land an off-screen source's dep ropes on the USER'S active project instead of the source's
      // own (the exact reach-around control-no-travel.source.test.ts pins against). `surface.addRope`
      // mints the same `ctrl-<dep>-<node>` id — live it hits controlEdges, off screen it commits to
      // the owning project's store.
      const ropeDeps = (ids: string[], after: string[] | undefined): void => {
        if (!after?.length) return
        for (const nid of ids)
          for (const dep of after) if (dep !== sourceNodeId) surface.addRope(dep, nid, edgeColor)
      }
      ...
      try {
        switch (verb) {
          ...
          case 'open-agent': {
            ...
            const after = resolveAfter()
            if (after === null) return // bad --after, already replied
            // `--auto-close yes`: the station closes itself once done AND read by its opener over
            // the context link (spawnedAlerts.ts `shouldAutoClose`). Only hook-reporting agents can
            // ever be `done`, so for anything else the node would simply never close — refuse up
            // front, like `--after` refuses a plain-terminal dependency.
            // The DEFAULT is the setting (on): the station is released by its opener's read unless
            // the caller says `--auto-close no` — a station it will keep talking to (spawnedAlerts.ts
            // `resolveAutoClose`). The capability refusals below are owed only to an EXPLICIT yes;
            // a defaulted arm on a pair that could never close is simply not armed. (`--project`
            // to ANOTHER project never reaches here — its branch returns above — so a defaulted
            // arm always lands in the caller's own canvas, where the consent lives.)
            const autoCloseChoice = resolveAutoClose(
              args['auto-close'],
              useSettings.getState().settings.autoCloseSpawnedNodes
            )
            const conductorAgent = surface.agentIdOf(sourceNodeId)
            const autoCloseCapable =
              hasHooks(agentId) &&
              canContextLink(agentId) &&
              !!conductorAgent &&
              canContextLink(conductorAgent)
            const wantAutoClose =
              autoCloseChoice.wanted && (autoCloseChoice.explicit || autoCloseCapable)
          ...
          case 'verify': {
            ...
            // its own relation, so each held wait gets its own dep rope and the group reads as the
            // DAG it is: every reviewer waits on the target, the verdict waits on every reviewer.
            ...
            // The panel follows the auto-close default too, every node armed to the CALLER: the
            // verdict node closes once the conductor has read it; a reviewer closes only if the
            // conductor reads that reviewer directly (it usually reads the verdict), and is otherwise
            ...
            reply({
              ...
              message:
                ...
                (panelArmed ? '\nauto-close armed: each panel node closes once done AND read by you' : '') +
                surface.queuedNote,
              result: {
                groupId: vGroup.id,
                targetId,
          ...
          case 'spawn-team': {
            ...
            const roleLaunches = await Promise.all(
              roles.map((r) => (r.promptFile ? spillLongPrompt(undefined) : spillLongPrompt(r.prompt)))
            )
          ...
          case 'color': {
            ...
            const colored = ids.filter((id) => surface.nodes().some((node) => node.id === id))
            if (!colored.length) {
              reply({ ok: false, error: 'color: none of the given node ids exist' })
              return
            }
            const selected = new Set(colored)
            // `surface.setNodes` + `surface.markDirty` write through the live canvas on screen and
            // through the owning project's store (via `persist`, never a bare `writeDisk`) off it —
            // one verb body, both surfaces, same as every other node mutation here.
            surface.setNodes((nodes) =>
              nodes.map((node) =>
                selected.has(node.id) ? { ...node, data: { ...node.data, color } } : node
              )
            )
            surface.markDirty()
            ...
            reply({
              ok: true,
              message: `colored ${colored.length} node(s) ${color}${note}` + surface.offscreenNote,
          ...
          case 'close': {
            // Targets: `--node <id,id>` (one or many) and/or `--spawned yes` = every node this
            // caller opened via canvas control that still exists (its outgoing ropes — the same
            // lineage the alert routing reads). ONE confirm dialog for the whole set: a 15-node
            // teardown as 15 serial dialogs measured as "no answer within 120s" ×1 + "a confirmation
            // is already pending" ×14 — the shape that made agents leave their stations behind.
            const wantSpawned = !!args.spawned && args.spawned !== 'no' && args.spawned !== 'false'
            const listed = (args.node ?? '')
              .split(',')
              .map((s) => s.trim())
              .filter(Boolean)
            if (!listed.length && !wantSpawned) {
            ...
            const spawned = wantSpawned
              ? surface.ropes()
                  ...
                  .map((r) => r.target)
              : []
            const targets = Array.from(new Set([...listed, ...spawned])).filter(
              (id) => live.has(id) && id !== sourceNodeId
            )
            if (!targets.length) {
              reply({
                ok: true,
            ...
            // The teardown, extracted so the waived path and the confirmed path are the SAME code
            // (see `write`'s `runWrite`). Canonical teardown through the fork's ControlSurface:
            // `deleteNodes()` destroys the local tmux session (remote-guarded), drops persisted
            // agentStatus, and reparents any group children; off screen the surface answers from
            // the owning project's store without moving the user's view. Don't hand-roll it.
            const runClose = (): void => {
            ...
            if (closeWaiver.via) {
              ...
              setNotice({
                ...
                text: waivedNotice(
                  targets.length === 1
                    ? `Agent "${srcTitle}" closed ${targets[0]}`
                    : `Agent "${srcTitle}" closed ${targets.length} nodes`,
                  closeWaiver.via,
                  ctlProject?.name
                )
              })
  ...
  useEffect(() => {
    ...
    return api.onAgentStatus((e: NormalizedAgentEvent) => {
      ...
      // conductor node. No unread flag: the conductor is the one still working, and its stations
      // were read by it, not by the user.
      const aggregateAlert = (spawner: string, finished: number, total: number): void => {
        const spawnerAgent = cs.byId[spawner]?.agentId
        const spawnerLabel = (spawnerAgent && agentConfig(spawnerAgent)?.label) || 'Agent'
        const body =
          total === 1
            ? `The station ${spawnerLabel} opened has finished.`
            : `${finished} of ${total} stations ${spawnerLabel} opened have finished.`
        const snd = useSettings.getState().settings
        if (snd.soundEffects) {
          const key = `fanout-${spawner}`
          const t = Date.now()
          if (t - (sfxCooldownRef.current[key] ?? 0) >= 5000) {
            sfxCooldownRef.current[key] = t
            playSfx('done', snd.soundVolume)
          }
        }
        if (document.hasFocus()) return
        if (!(snd.notifyOnClaudeDone && snd.notifyConsentAsked)) return
        const key = `fanout-${spawner}`
        const now = Date.now()
        if (now - (notifyCooldownRef.current[key] ?? 0) < 5000) return
        notifyCooldownRef.current[key] = now
        void window.nodeTerminal.notify({
          title: `${contextFor(spawner)} — ${spawnerLabel}'s stations finished`,
          body,
          nodeId: spawner
        })
      }
      ...
      switch (e.kind) {
        case 'state': {
          ...
          if (e.state === 'done' && !e.interrupted && !stuckRescueSkip && !wasAlreadyDone) {
            ...
            // A node an AGENT opened reports to that agent, not to the human: quiet while its
            // siblings still run, ONE aggregate alert on the conductor when the last one lands.
            ...
            const decision = useSettings.getState().settings.quietSpawnedNodes
              ...
              : ({ kind: 'alert' } as const)
            // #708 layers ON TOP of the fork's aggregate routing: even when this node's own
            // completion would alert, stay QUIET (chime + OS notification, never the unread dot)
            // while its in-process subagents are still working — the parent's "finished" is
            // premature until the fan-out it is running has landed. Computed from the LIVE store,
            // not the `an` snapshot taken at listener entry.
            if (decision.kind === 'alert')
              alert('finished', `${agentLabel} finished its turn.`, 'done', { quiet: fanoutStillWorking(Object.values(useAgentNodes.getState().byId), e.nodeId) })
            else if (decision.kind === 'aggregate')
              aggregateAlert(decision.spawner, decision.finished, decision.total)
          }
          if (e.state === 'blocked')
  ...
  // `--auto-close yes`: a station a conductor opened closes itself once it is DONE and its own
  // spawner has read it over a context link since (both, in that order — spawnedAlerts.ts
  // `shouldAutoClose`). No confirm dialog: the flag was armed by this process when the conductor
  ...
  // THE IDLE SWEEP — the backstop for the two cases auto-close cannot reach (spawnedAlerts.ts
  // `sweepFinishedStations`): an app restart (the arming is in-memory by design; tmux continuity
  // keeps the sessions) and a conductor that read its results elsewhere (git, files). Every
