    ...
    void (async () => {
      ...
      transport
        ...
        if (data.initialCommand) {
          ...
          agentId &&
          canResume(agentId) &&
          !data.pendingLaunch && // ≡ mayRelaunchAgent(data), spelled out: session-ready-signal.test pins it
          ...
          // `--resume`, is what must create it. Skip here and let the fire effect deliver (the same
          // gate `mayRelaunchAgent` applies to the restart paths). A PAUSED node is the other
          // hold-out (`shouldColdResume`, see `pausedNow` above): only an explicit Resume relaunches it.
